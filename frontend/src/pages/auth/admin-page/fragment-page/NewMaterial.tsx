...
type NewMaterialResponse = {
  comment: string;
};

const NewMaterial = ({ returnTop }: NewMaterialProps) => {
  ...
  const handle = async () => {
    ...
    try {
      const response = await axios.post<NewMaterialResponse>("http://localhost:8080/api/material/new", {
        name,
        destination,
      });
